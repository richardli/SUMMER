...
```{r}
out1 <- getSmoothed(fit1, is.yearly = FALSE)
out2 <- getSmoothed(fit2, is.yearly = TRUE)
```
...
fit3 <- fitINLA(data = data, geo = geo, Amat = mat, year_names = years.all,
                year_range = c(1985, 2019), priors = priors, rw = 2,
                is.yearly=FALSE)
out3 <- getSmoothed(fit3, Amat = mat, is.yearly = FALSE)
```
...
out4 <- getSmoothed(fit4, Amat = mat, is.yearly = TRUE)
```
