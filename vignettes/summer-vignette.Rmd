...
\begin{eqnarray*}
\hat\mu_i &\sim & \mbox{Normal}( \mu_i, \widehat{var}(\hat{\mu_i}) )\\
...
```{r}
fit <- fitSpace(data=DemoData2, geo=DemoMap2$geo, Amat=DemoMap2$Amat, family="gaussian", responseVar="height", strataVar="strata", weightVar="weights", regionVar="region", clusterVar = "~clustid+id", hyper=NULL, CI = 0.95)
```
...
The posterior summaries of $\mu_i|\hat\mu_i$ can be obtained by 
```{r}
...
mapPlot(data = combined, geo = DemoMap2$geo, variables=c("truth", "HT.est", "median"), labels = c("True mean", "Direct Estimates", "Posterior Median"), by.data = "region", by.geo = "NAME_final", is.long=FALSE)
```
