...
```{r}
Education <- svyby(~age, ~hracode, design, svymean)
Education <- Education[, 1:2]
svysmoothed_with_covariate <- fitSpace(data = BRFSS, geo = KingCounty, Amat = mat, X = Education, family = "binomial", responseVar="diab2", strataVar="strata", weightVar="rwt_llcp", regionVar="hracode", clusterVar = "~1", hyper=NULL,  CI = 0.95)
summary(svysmoothed_with_covariate$fit)
```
...
Next, we obtain Horvitz-Thompson estimators using `countrySummary_mult`. We specify the survey design as the popular two-stage stratified cluster sampling where strata are specified in the ``strata`` column, and clusters are specified by both the cluster ID (`clusterid`) and household ID (`id`). 


```{r, warning=FALSE}
years <- levels(DemoData[[1]]$time)

data_multi <- countrySummary_mult(births = DemoData, years = years,regionVar = "region", 
...
```{r}
plot(out4, is.yearly=TRUE, is.subnational=TRUE,  data.add = data_multi, option.add = list(point = "u5m", by = "surveyYears")) + facet_wrap(~region, scales = 'free') 
...
mapPlot(data = subset(out4, is.yearly==F), geo = DemoMap$geo,
        variables=c("years"), values = c("med"), by.data = "region", by.geo = "NAME_final", is.long=TRUE)
```
